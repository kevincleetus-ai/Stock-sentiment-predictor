...
# normalize dates so we can merge properly
...
# convert sentiment labels to numbers so the model can use them
sentiment_map = {"positive": 1, "neutral": 0, "negative": -1}
sentiment_df["sentiment_score"] = sentiment_df["sentiment"].map(sentiment_map)

# average sentiment per day in case there are multiple headlines
...
# 1 if price went up next day, 0 if it went down
